...
class TomlEditorDialog(tk.Toplevel):
    ...
    def __init__(self, parent, toml_path: str, on_save: Callable | None = None):
    ...
    def _load_toml(self):
        ...
        try:
            with open(self._toml_path, encoding="utf-8") as f:
                content = f.read()
    ...
    def _build_ui(self):
        ...
        self._authors_var = self._add_field(
            "Authors",
            ", ".join(pkg.get("authors", []))
            if isinstance(pkg.get("authors"), list)
            else pkg.get("authors", ""),
        )
    ...
    def _add_dependency_row(self, name: str, version: str):
        ...
        rm_btn.bind(
            "<Button-1>", lambda e, r=row, pair=(name_entry, ver_entry): self._remove_dep(r, pair)
        )
